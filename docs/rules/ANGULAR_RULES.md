...
5. Page architecture objective: use `pages/{role}` as the boundary. Any component that is only used by a specific role must be located under `pages/{role}` and not shared outside that role folder.
6. Must use `cd {folderPath}` and use `ng g c {component-name}` instead of creating component manually.

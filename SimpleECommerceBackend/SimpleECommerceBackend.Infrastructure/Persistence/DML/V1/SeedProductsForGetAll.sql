...
IF NOT EXISTS (SELECT 1 FROM [business].[UserProfiles] WHERE [Id] = @AdminId)
BEGIN
...
BEGIN
    ...
    (
        [Id], [Name], [Description], [AverageRating], [TotalRatings], [CategoryId], [SellerId], [CreatedAt], [UpdatedAt]
    )
    VALUES
    (@ProductMouseId, N'Wireless Mouse', N'Comfortable wireless mouse with silent click switches', 4.50, 12, @CategoryElectronicsId, @SellerId, SYSDATETIMEOFFSET(), NULL),
    (@ProductPlanterId, N'Ceramic Planter', N'Ceramic planter for indoor decoration', 4.80, 8, @CategoryHomeId, @SellerId, SYSDATETIMEOFFSET(), NULL);
END
